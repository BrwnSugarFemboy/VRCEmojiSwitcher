...
def OOBE():
	...
	if((os.path.exists(VRCEmojiSwitcherPath) is False)):
		...
		os.mkdir(VRCEmojiSwitcherPath + "\\Emojis\\3")
		os.mkdir(VRCEmojiSwitcherPath + "\\Emojis\\4")
		os.mkdir(VRCEmojiSwitcherPath + "\\Emojis\\5")
		os.mkdir(VRCEmojiSwitcherPath + "\\Emojis\\6")
...
def Authenticate():
	...
	with vrchatapi.ApiClient(configuration) as api_client:
		api_client.user_agent = "VRChatEmojiSwitcher / 0.1.4 femboy@bussy.wtf"
		...
		print("Logged in as: " + str(current_user.display_name))
...
def EmojiBackup(api_client=None):
	...
	for i in currentEmojis:
		...
		image = files_api.download_file_version(i.id, 1)
...
def RemoveAllEmojis(api_client=None):
	...
	for i in currentEmojis:
		...
		LogWriter("Emoji removed. Manual delay of 3s.")
		time.sleep(5)
...
def DirectoryTracker(currentDir=None):
	...
	if allDirs is None:
		...
		LogWriter("-----\nAll Files and Folders in Emoji Path: ")
	for item in allDirs:
	...
	if currentDir is None:
		currentDir = "1"
		nextDir = "1"
		LogWriter("Set nextDir to 1 because currentDir was None")
	else:
	...
	for i in allNewEmojis:
		if(ImageSizeValidation(i)):
			...
			if((len(EmojiSplitName) == 3) and (str(EmojiSplitName[1]).lower() == 'static') and (int(EmojiSplitName[0]) == allNewEmojis.index(i))):
				...
				files_api.upload_image((i), tag="emoji", animation_style=str(os.path.splitext(EmojiSplitName[2])[0]))
				time.sleep(5)
				...
				files_api.upload_image((i), tag="emojianimated", frames=int(EmojiSplitName[2]), frames_over_time=int(EmojiSplitName[3]), animation_style=str(os.path.splitext(EmojiSplitName[4])[0]))
				time.sleep(5)
			else:
...
def LogWriter(LogString=None):
	...
	if os.path.getsize(LogPath) > 10000000:
		if len(AllLogs) == 6:
			...
			os.rename(LogPath, (VRCEmojiSwitcherPath + "\\VRCEmojiSwitcherLog." + len(AllLogs) + ".log"))
	#Write to LogFile
